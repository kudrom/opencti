...
import { elBatchIds, elFindByIds } from '../../database/engine';
...
export const workspaceWidgetFiltersToStandardIds = async (
  context: AuthContext,
  _: AuthUser,
  manifest: string,
) => {
  const exportedWidget = JSON.parse(
    fromBase64(manifest) || '',
  );
  const widgetWithStandardIdsFilters = await convertToStixWidget(exportedWidget, context);
  return toBase64(JSON.stringify({
    ...widgetWithStandardIdsFilters,
...
const convertToStixWidget = async (
  widgetConfiguration: WidgetConfiguration,
  context: AuthContext,
) => {
  const [filters, dynamicFrom, dynamicTo] = widgetConfiguration.dataSelection?.flatMap(
    ({
      filters,
      dynamicFrom,
      dynamicTo,
    ...
    }) => [filters, dynamicFrom, dynamicTo],
  );

  const internalIds = [
    ...filters.filters?.flatMap(({ values }) => values.filter((value) => isInternalId(value)),),
    ...dynamicFrom.filters?.flatMap(({ values }) => values.filter((value) => isInternalId(value)),),
    ...dynamicTo.filters?.flatMap(({ values }) => values.filter((value) => isInternalId(value)),),
  ];

  const standardIdsByInternal = new Map(internalIds.map((id) => [id, '']));
  if (internalIds.length) {
    const entities = await elBatchIds(context, context.user, internalIds);
    entities.forEach(
      ({
        internal_id,
        standard_id,
        ...
        internal_id: string;
        standard_id: string;
      }) => standardIdsByInternal.set(internal_id, standard_id),
    );
  }

  const dataSelection: DataSelection[] = [
    ...widgetConfiguration.dataSelection.map((dataSelection) => ({
      ...dataSelection,
      filters: {
        ...dataSelection.filters,
        filters: [
          ...dataSelection.filters.filters.flatMap((filter) => ({
            ...filter,
            values: [
              ...filter.values.flatMap((internalId) => standardIdsByInternal.get(internalId) || internalId,),
            ],
          })),
        ],
      },
  ...
  return { ...widgetConfiguration, dataSelection };
};
